...
<html lang="ru">
  ...
  <body style="color:#ffffff">
    ...
    $query = mysqli_query($link, "SELECT ProfilePhoto FROM information WHERE id = $DB_id LIMIT 1");
    $row = mysqli_fetch_assoc($query);

    if($row['ProfilePhoto'] != NULL)
    {
      $dbcon = mysqli_connect("localhost", "admin", "danilstarosta", "school students");
      $query = "SELECT ProfilePhoto from information WHERE ID = '".$DB_id."'";
    ...
    function saveimage($image, $DB_id){
      ...
      $dbcon = mysqli_connect("localhost", "admin", "danilstarosta", "school students");
      $query = "UPDATE information SET ProfilePhoto = '$image' where ID = '".$DB_id."'";
